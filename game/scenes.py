...
class MenuScene(Scene):
    def __init__(self, window):
        ...
        # Create a
        title_image = pyglet.resource.image('SwarmConstructionLogo.png')
        ...
        self.start_label = pyglet.text.Label('Select blueprint to load & press Enter to start', font_size=25,
                                             x=self.window.width // 2, y=self.window.height // 2,
                                             anchor_x='center', anchor_y='center', batch=self.batch)

        # Create labels for three save slots:
        self.save_slot_labels = []
        for save_slot in [1, 2, 3, 4]:
            self.scene_manager.save.save_slot = save_slot
            # indicate if an existing save exists
            if save_slot == 4:
                label_text = f"{save_slot}:  Create new blueprint"
            elif self.scene_manager.save.has_save_game():
                label_text = f"{save_slot}:  Load existing blueprint"
            else:
                label_text = f"{save_slot}:  Create new blueprint"
            y_pos = 250 - 50 * save_slot
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol in (key._1, key._2, key._3, key._4):
            if symbol == key._1:
                ...
                self.scene_manager.save.save_slot = 4
            self._highlight_save_slot()
...
class GameScene(Scene):
    def __init__(self, window):
        ...
        self.block_group = BlockGroup(
            self.window, pyglet.resource.texture('textures_blocks.png'), order=0)
        ...
        # Size of the blueprint to be created
        self.map_size = 10
    ...
    def collide(self, position, height):
        ...
        pad = 0.25
        p = list(position)
        norm_p = normalize(position)
        for face in FACES:  # check all surrounding blocks
            for i in range(3):  # check each dimension independently
                if not face[i]:
                    continue
                # How much overlap you have with this dimension.
                d = (p[i] - norm_p[i]) * face[i]
                if d < pad:
                    continue
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self.exclusive:
            ...
            if button == mouse.RIGHT or (button == mouse.LEFT and modifiers & key.MOD_CTRL):
                ...
                if texture != BEDSTONE and block[1] != 0:
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        if symbol == key.W:
            ...
            self.map_size += 1
            generate_world(self.model, self.map_size)
        elif symbol == key.MINUS:
            self.map_size -= 1
            generate_world(self.model, self.map_size, smaller_size=True)
            ...
            for x, y, z in self.model.world.keys():
                x_vals.append(x)
                y_vals.append(y)
                z_vals.append(z)
            ...
            blueprint = np.zeros((x_size+1, z_size+1, y_size+1))
            for x, y, z in self.model.world.keys():
                blueprint[x, z, y] = 1

            print("Saving blueprint as .npy file. Remember to copy this file to the simulator")
            from file_config import location_to_save
            from uuid import uuid4
            np.save(location_to_save + "blueprint_" + str(uuid4()), blueprint)
    ...
    def draw_label(self):
        ...
        """
        x, y, z = self.position
        self.info_label.text = 'COORDS = [%.2f, %.2f, %.2f] : CURRENTLY SHOWN %d / TOTAL NUMBER OF ' \
                               'BLOCKS %d' % (
            x, y, z,
...
class HelpScene(Scene):
    def __init__(self, window):
        ...
        self.text_strings = ["  GAME OPTIONS",
                             ...
                             "* Press P key to save model",
                             "* Press + key to extend model base by 1 in x-y plane",
                             "* Press - key to shorten model base by 1 in x-y plane",
